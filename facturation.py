...
if df_chambres.empty:
    st.warning("Aucune chambre configurée pour le moment. Rends-toi sur la page Chambres "
               "pour en ajouter avant de pouvoir facturer.")
    st.stop()
...
with tab_nouvelle:
    ...
    with col_form:
        ...
        room_data = db.get_chambre(etab_id, room_facture)
    ...
    with col_recap:
        ...
        if "derniere_facture" in st.session_state:
            ...
            colp.download_button(
                "⬇️ PDF", data=generer_pdf_facture(f, NOM_ETABLISSEMENT),file_name=f"{f['numero_facture']}.pdf", mime="application/pdf", key="dl_pdf_derniere"
            )
        ...
        for _, f in df_factures.iterrows():
            ...
            with st.expander(
                ...
                colb2.download_button(
                    ...
                    file_name=f"{f['numero_facture']}.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",key=f"docx_{f['numero_facture']}"
                )
                ...
                if f['statut_paiement'] == "En attente":
                    if colb4.button("Marquer payé", key=f"payer_{f['numero_facture']}"):
                        db.maj_statut_paiement(etab_id, f['numero_facture'], "Payé")
                        st.rerun()
